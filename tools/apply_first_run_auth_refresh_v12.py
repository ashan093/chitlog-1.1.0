...
def _authentication_assignment(
    ...
    for node in ast.walk(scope):
        if _scope(node, parents) is not scope:
            continue
        # Python 3.14 AST includes helper nodes such as ast.arguments that do
        # not carry source-position attributes. Filter to assignment nodes
        # before reading lineno/col_offset.
...
def patch_application(path: Path) -> bool:
    ...
    if MARKER in source:
        print("First-run authentication refresh v12 is already installed.")
        return False
    ...
    backup = path.with_suffix(path.suffix + ".pre_first_run_auth_refresh_v12.bak")
